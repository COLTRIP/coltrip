...
장소명 검색은 좌표가 필요 없는 `GET /api/spots/search?keyword=해양&page=0&size=20`을 사용한다. 부분 일치, 이름/ID 오름차순, 기본 20개(최대 50개)이며 비로그인 허용이다. 응답은 기존 장소 요약과 페이지 메타데이터를 포함한다. 빈 결과는 200과 spots=[]이며 이미지/고요지수 없는 장소도 포함한다. 입력/오류 및 프론트 계약은 [검색 API 명세](./spot-search-api-spec.md)를 참고한다.
