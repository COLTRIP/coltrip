...
- FCM 빈, 백그라운드 방문 재평가(Nudge)는 시연 활성 시 등록하지 않는다. FCM 자격증명이 지정되면 기동도 거부한다. 테스트 기기에도 푸시를 보내지 않는 정책이다.
- AI 고요지수 지도 자동 동기화(`QuietIndexMapSyncScheduler`)는 **시연 환경에서도 켜져 있다**(2026-09-16 변경, 이슈 #127). AI에 읽기 전용(GET)으로만 요청하고 이 프로필의 DB(`coltrip_demo`)만 갱신하므로 운영 데이터 격리를 깨지 않으면서도, 시연 서버가 실제로 실시간 고요지수를 반영하게 한다. 운영과 동일한 기본 주기(매시 5분)로 동작하며, `AI_BASE_URL`/`AI_API_KEY` 환경변수가 필요하다(운영과 같은 값을 써도 무방 - 읽기 전용 호출이라 위험하지 않다).
...
DemoConfigurationTest는 환경/DB/키/허용 목록 및 초기화 이전 차단을 검증하고 DemoModeIntegrationTest는 계정 제한, 발급 차단, 게스트 세션 발급과 즉시 사용, 부산 추천과 방문 흐름, 소유권/상태, FCM/Nudge 비활성화와 고요지수 지도 동기화 활성화를 검증한다. DemoDisabledIntegrationTest는 운영 프로필에 게스트 세션 엔드포인트 자체가 없음(404)을 확인한다. H2 테스트는 MySQL 권한 격리나 실제 기기의 가상 GPS/반경 판정 검증을 대신하지 않는다.
