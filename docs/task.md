...
- [x] 백그라운드 푸시 1단계(#89, 2026-09-15): FCM 연동(미설정 시 자동 비활성화), 기기 토큰 등록/해제 API, 진행 중 방문 주기 평가(기존 check와 동일 판정 재사용), 같은 제안 중복발송 방지, 무효 토큰 자동 정리. 상세: [푸시 명세](./push-notification-spec.md)
- [ ] 발송 실패 재시도 정책, 다중 기기 세분화는 2단계로 보류.
- [ ] 프론트 알림 권한/수신/딥링크 연동 및 실제 FCM 프로젝트(서비스 계정) 발급·운영 반영.
