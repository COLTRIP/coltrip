...
## 블로커 — 이것부터 확인해야 다음 단계 진행 가능

- [ ] `category`(장소유형) enum 최종 값 확정 (schema.md 참고)
- [ ] 대체지 추천: 배치 저장 vs AI 실시간 호출 확정
- [ ] `similarityScore` 스케일 AI팀과 통일
- [ ] QuietIndex 배치 계산 주기 확정 (AI팀 담당이지만 백엔드는 캐시 갱신 시점 관련해서 인지 필요)
...
## Phase 2 — 인증 ([api.md](./api.md) [인증] 섹션)

- [ ] Spring Security 설정
- [ ] `POST /api/auth/google` — 구글 idToken 검증 → User 조회/생성 → JWT 발급
- [ ] `POST /api/auth/refresh` — 리프레시 토큰 검증/재발급
- [ ] `POST /api/auth/logout` — 리프레시 토큰 무효화
- [ ] JWT 인증 필터 (Authorization 헤더 검증, SecurityContext 등록)
- [ ] 예외 처리: `InvalidGoogleTokenException`, `InvalidRefreshTokenException`, `UnauthorizedException`
