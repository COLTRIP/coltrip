...
값이 없는 슬롯은 null이다(0점이나 인접 날짜 값으로 대체하지 않음). AI가 하루 1값(15시 고정)만 제공하는 현재 관례([위 참고](#운영-관례--현재-예측은-하루-1값2026-09-16))상, 실질적으로는 `hour=15`로 조회해야 값이 채워진다.

**검증 규칙은 24시간 버전과 다르다(2026-09-20 수정, 이슈 #134 피드백).** 24시간 버전은 date+hour가 만드는 정확한 시각이 이미 지났는지를 검증하지만, 이 API의 hour는 그 날의 대표 슬롯일 뿐 실시간 의미가 없다(AI가 하루 1값만 주는 현재 관례). 그래서 시각이 아니라 **날짜 단위**로만 검증한다 — `date`가 오늘보다 이전이면 거부하고, `date`가 오늘이면 대표 시각(예: 15시)이 이미 지난 시각이어도 거부하지 않는다. 상한도 마찬가지로 날짜 단위다 — 마지막 슬롯의 날짜(`date`+6일)가 오늘부터 7일 뒤보다 늦으면 거부한다.
