...
public class CurrentRecommendationController {
    ...
    @Operation(summary = "현재 고요지수 기준 추천", description = """
            ...
            radiusMeters 100~50000, limit 1~50(기본 20). category/mode 필터를 적용합니다.
            점수 내림차순(null 마지막), 거리/ID 오름차순이며 미수신 점수는 0이나 예측값으로 대체하지 않습니다.
    ...
    @ApiResponse(responseCode = "400", description = "위치/반경/개수 또는 파라미터 형식 오류")
    ...
    public ResponseEntity<CurrentRecommendationResponse> recommend(@AuthenticationPrincipal Long userId,
        ...
        return ResponseEntity.ok().cacheControl(CacheControl.noStore()).body(
                service.recommend(userId, latitude, longitude, radiusMeters, category, mode, limit));
    }
